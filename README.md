...
6. 系统默认内置一份示例返回数据，打开页面即可直接看到完整效果。

---

## 2. 部署说明

> 本次更新（移除登录界面）**不需要新增数据库表**，仍使用原有 `users`、`workflow_records` 与六模块表结构。后端会自动使用系统内置用户 `system_guest` 写入记录。
...
1. 直接打开页面，输入 `workflow token` 与用户描述。
2. 点击“生成投资方案”，系统会请求工作流并落库。
